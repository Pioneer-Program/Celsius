...
import cute.ame.celsius.Thermal.Data.ThermalStore;
import cute.ame.celsius.Thermal.Helper.ThermalBreakdown;
import cute.ame.celsius.Thermal.Level.ThermalLevelData;
import cute.ame.celsius.Thermal.Physics.ThermalConduction;
import cute.ame.celsius.Thermal.Physics.ThermalRadiation;
...
public final class ThermalTickEvents
{
    ...
    public static int pass(ServerLevel level, ThermalLevelData data, int period)
    {
        ...
        float sink = Ambient.of(level).vacuum() ? Config.THERMAL_SPACE_SINK_K.get().floatValue() : ambient;
        ...
        for (int n = 0; n < budget; n++)
        {
            ...
            if (!Residency.isLoaded(level, centre)) continue;
            ...
            for (Direction face : FACES)
            {
                ...
                if (nslot != ThermalStore.INVALID)
                {
                    ...
                    {
                        if (!Residency.isLoaded(level, probe)) continue;
