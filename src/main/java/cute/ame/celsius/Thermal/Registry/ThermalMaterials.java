package cute.ame.celsius.Thermal.Registry;
...
public final class ThermalMaterials
{
    ...
    public static final ResourceLocation DEFAULT_ID = ResourceLocation.fromNamespaceAndPath(Celsius.MODID, "default");
    ...
    public static void refresh(MinecraftServer server)
    {
        ...
        Celsius.LOGGER.info("[Celsius] Thermal material table: {} material(s) over {} block(s)", count, index.size());
    }
    ...
    private static BlockState resolveState(Optional<String> id, HolderLookup.RegistryLookup<Block> blocks)
    {
        ...
        if (parsed == null)
        {
            Celsius.LOGGER.warn("[Celsius] Thermal material: malformed breakdown_result '{}'", id.get());
            return Blocks.AIR.defaultBlockState();
        }

        return blocks.get(ResourceKey.create(Registries.BLOCK, parsed)).map(holder -> holder.value().defaultBlockState()).orElseGet(() ->
        {
            Celsius.LOGGER.warn("[Celsius] Thermal material: unknown breakdown_result '{}'", parsed);
            return Blocks.AIR.defaultBlockState();
        });
    }
    ...
    private static void bind(Reference2IntOpenHashMap<Block> index, int slot, String target, HolderLookup.RegistryLookup<Block> blocks, ResourceLocation owner)
    {
        ...
        if (id == null)
        {
            Celsius.LOGGER.warn("[Celsius] Thermal material {}: malformed target '{}'", owner, target);
            return;
        }
        ...
        if (isTag)
        {
            ...
            if (tag.isEmpty())
            {
                Celsius.LOGGER.debug("[Celsius] Thermal material {}: empty or absent block tag #{}", owner, id);
                return;
            }
        ...
        if (block.isEmpty())
        {
            Celsius.LOGGER.warn("[Celsius] Thermal material {}: unknown block '{}'", owner, id);
            return;
        }
