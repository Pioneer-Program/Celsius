...
public final class ThermalDevices
{
    ...
    public static void refresh(MinecraftServer server)
    {
        ...
        index.trim();
        byBlock = index;
        Celsius.LOGGER.info("[Celsius] Thermal devices: {} definition(s) over {} block(s)", raw.size(), index.size());
    }
    ...
    private static void bind(Reference2ObjectOpenHashMap<Block, ThermalDevice> index, ThermalDevice device, String target, HolderLookup.RegistryLookup<Block> blocks, ResourceLocation owner)
    {
        ...
        if (id == null)
        {
            Celsius.LOGGER.warn("[Celsius] Thermal device {}: malformed target '{}'", owner, target);
            return;
        }
        ...
        if (isTag)
        {
            ...
            if (tag.isEmpty())
            {
                Celsius.LOGGER.debug("[Celsius] Thermal device {}: empty or absent block tag #{}", owner, id);
                return;
            }
        ...
        if (block.isEmpty())
        {
            Celsius.LOGGER.warn("[Celsius] Thermal device {}: unknown block '{}'", owner, id);
            return;
        }
